...
const ClientsPage = {
  ...
  async showEditClient(clientId) {
    ...
    Modal.show({ title: 'Edit Client', body: `<form id="modal-form">
      <div class="form-row">
        <div class="form-group"><label>Full Name *</label><input name="full_name" required value="${escHtml(c.full_name)}"></div>
        <div class="form-group"><label>Mobile</label><input name="mobile" value="${escHtml(c.mobile || '')}"></div>
        <div class="form-group"><label>Notes</label><textarea name="notes">${escHtml(c.notes || '')}</textarea></div>
      </div>
    </form>`, onSave: async () => {
      if (!Modal.validate()) throw new Error('Full name is required');
